...
const quoteSamples = [
  { id: "architectural-floor-plan", name: "Architectural floor plan", image: "/samples/architectural-floor-plan.png", hint: "Floor plans, permit sets, room layouts" },
  { id: "engineering-site-plan", name: "Engineering site plan", image: "/samples/engineering-site-plan.png", hint: "Site plans, utilities, civil drawings" },
  { id: "color-presentation-board", name: "Color presentation board", image: "/samples/color-presentation-board.png", hint: "Elevations, render boards, presentations" },
  { id: "construction-detail", name: "Construction detail", image: "/samples/construction-detail.png", hint: "Sections, details, technical drawings" },
];
...
export default function ProductsPage() {
  ...
  async function submitQuote(event: React.FormEvent) {
    ...
    try {
      const sample = quoteSamples.find((item) => item.id === selectedSample);
      const response = await fetch("/api/quotes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productName: quoteProduct, customerName: formData.name, customerEmail: formData.email, customerPhone: formData.phone, details: `Selected sample: ${sample?.name || "No sample selected"}. ${sample?.hint || ""}\n\nCustomer requirements: ${formData.details || "Not provided"}` }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Unable to submit your request.");
      setSubmitted(true);
      setTimeout(() => { setSubmitted(false); setQuoteProduct(""); setSelectedSample(""); setFormData({ name: "", email: "", phone: "", details: "" }); }, 2200);
  ...
  return <main className="bg-slate-50"><Navbar />
    ...
    {quoteProduct && <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-7"><div className="flex justify-between items-start mb-6"><div><p className="eyebrow mb-1">Custom product quote</p><h3 className="font-display text-2xl font-bold text-gray-900">{quoteProduct}</h3></div><button aria-label="Close quote form" onClick={() => setQuoteProduct("")} className="p-2 rounded-lg hover:bg-gray-100"><X className="w-5 h-5" /></button></div>{submitted ? <div className="py-10 text-center"><CheckCircle2 className="w-14 h-14 text-green-500 mx-auto mb-3" /><h4 className="font-bold text-gray-900">Request received</h4><p className="text-gray-500 text-sm mt-1">We&apos;ll get back to you within 24 hours.</p></div> : <form onSubmit={submitQuote} className="space-y-5"><div><p className="font-bold text-gray-900">Choose a sample closest to your project</p><p className="text-sm text-gray-500 mt-1">This helps our sales team understand what you need. It is only a reference, not a final design.</p><div className="grid grid-cols-2 gap-3 mt-3">{quoteSamples.map((sample) => <button type="button" key={sample.id} onClick={() => setSelectedSample(sample.id)} className={`text-left rounded-2xl overflow-hidden border-2 transition-all ${selectedSample === sample.id ? "border-blueprint-600 ring-2 ring-blueprint-100" : "border-gray-200 hover:border-blueprint-300"}`}><img src={sample.image} alt={sample.name} className="w-full h-28 object-cover" /><div className="p-3"><p className="text-sm font-bold text-gray-900">{sample.name}</p><p className="text-xs text-gray-500 mt-1">{sample.hint}</p></div></button>)}</div></div><input required placeholder="Full name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3" /><input required type="email" placeholder="Email address" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3" /><input placeholder="Phone (optional)" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3" /><textarea rows={3} placeholder="Quantity, sizes, timeline, or special requirements" value={formData.details} onChange={(e) => setFormData({ ...formData, details: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3 resize-none" />{error && <p className="text-sm text-red-600">{error}</p>}<button disabled={submitting || !selectedSample} className="btn-primary w-full flex items-center justify-center gap-2 disabled:opacity-60"><Send className="w-4 h-4" />{submitting ? "Sending..." : "Send quote request"}</button>{!selectedSample && <p className="text-center text-xs text-gray-500">Select a sample above to continue.</p>}</form>}</div></div>}
  </main>;
}
