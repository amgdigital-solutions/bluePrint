...
export default function OrderPage() {
  ...
  const itemTotal = (item: CartItem) => item.quantity * priceFor(item.printType);
  const subtotal = Number(cart.reduce((sum, item) => sum + itemTotal(item), 0).toFixed(2));
  ...
  async function checkDistance() {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to calculate distance."); } finally { setBusy(false); }
  }
  ...
  async function submitOrder(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerName: form.name, customerEmail: form.email, customerPhone: form.phone, items: cart, deliveryAddress: delivery === "delivery" ? form.address : null, distanceMiles: delivery === "delivery" ? distance : null, deliveryChoice: delivery, fileName: form.file.name, fileUrl: blob.url, notes: form.notes }) });
      const result = await response.json();
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to submit your order."); } finally { setBusy(false); }
  }

  return (
    <main className="bg-slate-50"><Navbar />
      <section className="pt-32 pb-20"><div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="eyebrow mb-2">Cart & checkout</p><h1 className="font-display text-4xl lg:text-5xl font-bold text-gray-900 tracking-tight">Build your print order</h1><p className="text-gray-600 mt-3 mb-10">Choose one or both print types, then upload your file once at checkout.</p>
