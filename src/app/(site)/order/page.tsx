...
async function analyzePdf(file: File) {
  ...
  return { pageCount: Math.max(1, text.match(/\/Type\s*\/Page\b/g)?.length || 1), printType: "bw" as PrintType };
}
...
export default function OrderPage() {
  ...
  const totalSets = files.reduce((sum, file) => sum + file.sets, 0);
  const bindingSets = totalSets > 1 ? totalSets : 0;
  ...
  const grandTotal = Number((taxableTotal + taxAmount).toFixed(2));
  ...
  async function submitOrder(event: React.FormEvent) {
    ...
    try {
      const uploadedFiles = [];
      for (const item of files) { const safeName = item.file.name.replace(/[^a-zA-Z0-9._-]/g, "_"); const path = `orders/guest/${crypto.randomUUID()}-${safeName}`; const blob = await upload(path, item.file, { access: "private", handleUploadUrl: "/api/upload" }); uploadedFiles.push({ fileName: item.file.name, fileUrl: blob.url, printType: item.printType, pageCount: item.pageCount, sets: item.sets }); }
      const response = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerName: form.name, customerEmail: form.email, customerPhone: form.phone, files: uploadedFiles, printConsentAccepted: printConsent, bindingRequested, deliveryAddress: delivery === "delivery" ? form.address : null, alternateDeliveryAddress: member && addressMode === "alternate", distanceMiles: delivery === "delivery" ? distance : null, deliveryChoice: delivery, notes: form.notes }) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error || "Unable to submit your order.");
      if (!result.checkoutUrl) throw new Error("Secure payment checkout was not returned. Please contact Blueprints Club.");
  ...
  return <main className="bg-slate-50"><Navbar /><section className="pt-32 pb-20"><div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex items-center justify-between mb-5"><p className="eyebrow">Checkout</p><Link href="/" className="text-sm font-semibold text-blueprint-700 hover:text-blueprint-900">← Back to home</Link></div><h1 className="font-display text-4xl lg:text-5xl font-bold text-gray-900 tracking-tight">Upload your blueprint files</h1><p className="text-gray-600 mt-3 mb-10">Every uploaded PDF becomes part of your order. We count pages and suggest the print mode automatically.</p>
    <form onSubmit={submitOrder} className="grid lg:grid-cols-[1fr_360px] gap-8 items-start"><div className="space-y-6">
      ...
      <section className="bg-white rounded-3xl border border-gray-200 p-7"><div className="flex items-center gap-3 mb-6"><Truck className="w-5 h-5 text-blueprint-600" /><div><h2 className="font-display text-xl font-bold text-gray-900">Pickup or delivery</h2><p className="text-sm text-gray-500">Members: free delivery on $50+ orders within 10 miles. Non-members: $15 delivery fee within 10 miles.</p></div></div><div className="flex gap-3 mb-5"><button type="button" onClick={() => setDelivery("pickup")} className={`flex-1 rounded-xl border-2 px-4 py-3 text-sm font-bold ${delivery === "pickup" ? "border-blueprint-600 bg-blueprint-50 text-blueprint-700" : "border-gray-200 text-gray-900"}`}>Pickup at store</button><button type="button" disabled={!eligibleDelivery} onClick={() => setDelivery("delivery")} className={`flex-1 rounded-xl border-2 px-4 py-3 text-sm font-bold disabled:opacity-50 ${delivery === "delivery" ? "border-blueprint-600 bg-blueprint-50 text-blueprint-700" : "border-gray-200 text-gray-900"}`}>Eligible delivery</button></div>{member && <div className="flex gap-2 mb-4"><button type="button" onClick={() => { setAddressMode("profile"); setForm((current) => ({ ...current, address: profileAddress })); setDistance(null); if (profileAddress) void checkDistanceFor(profileAddress); }} className={`flex-1 rounded-xl border-2 px-3 py-2 text-sm font-bold ${delivery === "delivery" && addressMode === "profile" ? "border-blueprint-600 bg-blueprint-50 text-blueprint-700" : "border-gray-200 text-gray-900"}`}>Use saved address</button><button type="button" onClick={() => { setAddressMode("alternate"); setForm((current) => ({ ...current, address: "" })); setDistance(null); }} className={`flex-1 rounded-xl border-2 px-3 py-2 text-sm font-bold ${delivery === "delivery" && addressMode === "alternate" ? "border-blueprint-600 bg-blueprint-50 text-blueprint-700" : "border-gray-200 text-gray-900"}`}>Another address (+$15)</button></div>}{member && addressMode === "profile" && <p className="text-xs text-gray-500 mb-3">Saved member address: {form.address || "No address saved in profile"}</p>}<div className="flex gap-3"><input readOnly={member && addressMode === "profile" && Boolean(profileAddress)} placeholder={member && addressMode === "profile" ? "Saved profile address" : "Delivery address"} value={form.address} onChange={(event) => { setForm({ ...form, address: event.target.value }); setDistance(null); }} className={`flex-1 rounded-xl border border-gray-200 px-4 py-3 ${member && addressMode === "profile" ? "bg-gray-50" : ""}`} /><button type="button" onClick={checkDistance} disabled={busy || !form.address.trim()} className="rounded-xl bg-blueprint-600 text-white px-4 disabled:opacity-60">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <MapPin className="w-4 h-4" />}</button></div>{member && addressMode === "alternate" && <p className="text-xs text-amber-700 mt-2">An alternate delivery address adds $15 and must still be within 10 miles.</p>}{!member && <p className="text-xs text-blueprint-700 mt-2">Non-members can order delivery within 10 miles for a $15 delivery fee.</p>}{distance !== null && <p className={`mt-4 rounded-xl p-4 text-sm ${eligibleDelivery ? "bg-green-50 text-green-800" : "bg-amber-50 text-amber-900"}`}>{eligibleDelivery ? `Delivery available — ${distance} miles away${deliveryFee ? " · $15 delivery fee" : ""}.` : `Delivery is unavailable at ${distance} miles or because the subtotal is below $50. Choose pickup or call +1 561-804-9110.`}</p>}<textarea rows={3} placeholder="Special instructions (optional)" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3 mt-5 resize-none" /></section>
      <div className="space-y-3"><label className="flex gap-3 items-start rounded-2xl border border-blueprint-100 bg-blueprint-50 p-4 text-sm text-blueprint-950"><input type="checkbox" checked={printConsent} onChange={(event) => setPrintConsent(event.target.checked)} className="mt-1 h-4 w-4 accent-blueprint-600" /><span>I confirm that the B&amp;W or Color selection for each document is accurate. I understand Blueprints Club may contact me and charge the price difference if the selected mode does not match the uploaded file.</span></label></div>
    </div><aside className="lg:sticky lg:top-28 bg-blueprint-950 text-white rounded-3xl p-7 shadow-xl"><div className="flex items-center gap-3 mb-7"><ShoppingCart className="w-5 h-5 text-blueprint-300" /><h2 className="font-display text-xl font-bold">Order summary</h2></div>{checkoutCart.length ? <><div className="space-y-3 border-b border-white/10 pb-5">{checkoutCart.map((item) => <div key={item.printType} className="flex justify-between text-sm"><span className="text-blueprint-200">{item.quantity}× {prices[item.printType].label}</span><span>${(item.quantity * priceFor(item.printType)).toFixed(2)}</span></div>)}{bindingSets > 0 && <label className="flex gap-3 items-start rounded-xl border border-white/15 bg-white/5 p-3 text-sm"><input type="checkbox" checked={bindingRequested} onChange={(event) => setBindingRequested(event.target.checked)} className="mt-1 h-4 w-4 accent-blueprint-400" /><span><strong className="text-white">Blueprint Binding strip</strong><span className="block text-xs text-blueprint-200 mt-1">$1.99 per set × {bindingSets} eligible set{bindingSets === 1 ? "" : "s"}. Uncheck if you do not want binding.</span></span></label>}{bindingFee > 0 && <div className="flex justify-between text-sm"><span className="text-blueprint-200">Binding strips</span><span>${bindingFee.toFixed(2)}</span></div>}{deliveryFee > 0 && <div className="flex justify-between text-sm"><span className="text-blueprint-200">Delivery fee</span><span>$15.00</span></div>}<div className="flex justify-between text-sm"><span className="text-blueprint-200">Tax ({taxRate.toFixed(2)}%)</span><span>${taxAmount.toFixed(2)}</span></div></div><div className="flex justify-between pt-5"><span className="font-bold">Total</span><span className="font-display text-3xl font-bold">${grandTotal.toFixed(2)}</span></div>{member && <p className="text-sm text-green-300 mt-5">Member pricing applied automatically.</p>}<button disabled={busy} className="w-full mt-7 bg-white text-blueprint-900 rounded-xl py-3.5 font-bold disabled:opacity-60">{busy ? "Preparing secure checkout..." : "Continue to secure payment"}</button><p className="mt-3 text-xs text-blueprint-300">You will pay securely on Square. Your order is confirmed only after payment clears.</p></> : <div className="text-center py-8"><ShoppingCart className="w-10 h-10 text-blueprint-300 mx-auto mb-3" /><p className="text-blueprint-200 text-sm">Upload PDF files to build your order.</p></div>}{error && <p role="alert" className="mt-4 text-sm text-red-200">{error}</p>}<div className="flex gap-2 mt-5 text-xs text-blueprint-300"><Clock3 className="w-4 h-4" />Timing is confirmed based on workload and order complexity.</div><p className="mt-3 text-xs text-blueprint-300">Uploaded PDFs are private and automatically deleted after 30 days.</p></aside></form>
